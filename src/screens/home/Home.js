...
class Home extends Component {
  ...
  render() {
    ...
    let filteredData = this.state.filteredImgs.filter((img) => {
      if (typeof img.caption === "undefined") {
        img.caption = "Far Hills, NJ";
      }
      return (
        img.caption
          .toLowerCase()
  ...
  getBaseUserInfo = () => {
    let that = this;
    let url = `${constants.userInfoUrl}=${sessionStorage.getItem(
      "access-token"
    )}`;
    return fetch(url)
      ...
      .then((jsonResponse) => {
        ...
        this.state.userInfo.map((data, index) => this.getMediaData(data.id));
      })
  ...
  getMediaData = (id) => {
    let that = this;
    let url = `${
      constants.userMediaUrl
    }/${id}?fields=id,media_type,media_url,username,timestamp&access_token=&access_token=${sessionStorage.getItem(
      "access-token"
    )}`;
    return fetch(url)
      .then((response) => response.json())
...
class HomeItem extends Component {
  ...
  render() {
    ...
    if (captionText === "") {
      return <div className="home-item-main-container" />;
    } else {
      return (
